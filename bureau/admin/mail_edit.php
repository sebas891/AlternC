...
<?php if (($custom_delivery = variable_get("custom_delivery", "", "None by default")) != "") { $span = 3; } else { $span = 2; } ?>
<table class="tedit">
  <tr><th colspan="<?php echo $span; ?>"><b><?php __("Is this email enabled?"); ?></b></th></tr>
...
<td colspan="<?php echo $span-1; ?>">
  ...
  <tr><th colspan="<?php echo $span; ?>"><b><?php __("Is it a:"); ?></b></th></tr>
  <tr>    <td>
	<input type="radio" name="islocal" id="islocal1" class="inc" value="1"<?php !isset($new_account)?cbox($islocal==1):cbox($islocal==0); ?> onclick="popon()" /><label for="islocal1"><?php __("POP/IMAP account"); ?></label> </td><td>
	<input type="radio" name="islocal" id="islocal0" class="inc" value="0"<?php !isset($new_account)?cbox($islocal==0):""; ?> onclick="popoff();" /><label for="islocal0"><?php __("A redirection to other email address"); ?></label></td>
	<?php if ($custom_delivery != "") { ?><td><input type="radio" name="islocal" id="islocal2" class="inc" value="2" onclick="popon();" <?php cbox($islocal==2) ?>/><label for="islocal2"><?php __("A custom delivery") ; echo " ($custom_delivery)"; ?></label></td> <?php } ?>
  </tr><tr>
      <td style="width: 50%; text-align: justify"><?php __("POP/IMAP accounts are receiving emails in the server. To read those emails, you can use a Webmail, or a mail client such as Thunderbird. If you don't use POP/IMAP, you can configure your email to be a redirection to other existing emails. The maximum size is in megabytes, use 0 to make it infinite."); ?><br />
<p>&nbsp;</p>
<?php if ($islocal) { ?>
...
<?php } ?>
  ...
  <tr><td style="width: 50%; text-align: justify"><label for="recipients"><?php __("If you want to send emails received on this address to other addresses, even outside this server, enter those recipients here."); ?></label></td></tr><tr><td>(<?php __("one recipient per line"); ?>)<br /><textarea class="int" cols="32" rows="5" name="recipients" id="recipients"><?php echo $recipients; ?></textarea></td></tr> </table>
<?php 
...
?>
	<tr class="trbtn"><td colspan="<?php echo $span; ?>">
